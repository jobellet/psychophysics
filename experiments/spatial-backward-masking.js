...
function updateDerivedDvaMetrics() {
  ...
  try {
    ...
    TARGET_ECCENTRICITY_PX = Math.round(VisualAngle.dvaToPixels(TARGET_ECCENTRICITY_DVA, reference));
...
window.addEventListener('resize', () => { updateDerivedDvaMetrics(); });
...
function buildCsv(data) {
  ...
  const rows = data.map(row => columns.map(key => {
    const value = row[key];
...
async function runTrial(trialParams) {
  const { trialIndex, isTargetPresent, targetLocation, soaFrames } = trialParams;
  ...
  document.getElementById('hud-trial').textContent = `Trial ${trialIndex + 1} of ${TOTAL_TRIALS}`;
  ...
  const handlePointer = (event) => {
    ...
    // Check fixation
    const distFix = Math.hypot(responseX - stageCenter, responseY - stageCenter);
  ...
  if (isTargetPresent) {
    ...
    if (consecutiveHits >= 3) {
      currentStaircaseSoaFrames = Math.max(MIN_SOA_FRAMES, currentStaircaseSoaFrames - 1);
      consecutiveHits = 0;
    } else if (consecutiveMisses >= 3) {
      currentStaircaseSoaFrames = Math.min(MAX_SOA_FRAMES, currentStaircaseSoaFrames + 1);
      consecutiveMisses = 0;
    }
  } else {
...
function finalizeSession(status) {
  ...
  document.getElementById('hud-trial').textContent = status === 'complete' ? `Completed all trials` : 'Session stopped early';
}
...
downloadButton.addEventListener('click', () => {
  ...
  downloadBlob(`spatial-backward-masking_${ts}.csv`, buildCsv(csvData), 'text/csv');
  downloadBlob(`spatial-backward-masking_${ts}.json`, JSON.stringify(csvData, null, 2), 'application/json');
});
